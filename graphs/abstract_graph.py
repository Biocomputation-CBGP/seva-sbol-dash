...
class AbstractGraph:
    def __init__(self, graph=None):
        ...
        if isinstance(graph, nx.MultiDiGraph):
            ...
            self._graph, self._max_key = _rdf_to_networkx(graph)
        else:
            g = Graph()
            g.parse(graph)
            self._graph, self._max_key = _rdf_to_networkx(g)
    ...
    def add_graph(self, graph):
        self.add_new_edges([(graph.nodes[n]["key"], e[1], graph.nodes[v]["key"])
                           for n, v, e in graph.edges(keys=True)])

    def set_max_key(self, new):
        self._max_key = new
    ...
    def add_new_edges(self, triples):
        max_key = self.max_key

        def _add_node(entity):
            nonlocal max_key
            if entity in nodes.keys():
                n_key = nodes[entity]
            else:
                n_key = max_key
                nodes[entity] = n_key
                max_key += 1
            self.add_node(n_key, key=entity)
            return n_key

        nodes = {v["key"]: k for k, v in self.nodes(data=True)}
        for s, p, o in triples:
            n = _add_node(s)
            v = _add_node(o)
            props = {"display_name": self._get_name(o)}
            self.add_edge(n, v, key=(s, p, o), **props)
        self.set_max_key(max_key)
        self._generate_labels()
    ...
    def _generate_labels(self):
        ...
        for subject, node, edge in self.edges:
            ...
            if "display_name" not in edge_data.keys():
                nx.set_edge_attributes(self._graph, {(subject, node, edge): {
                                       "display_name": self._get_name(edge[1])}})
