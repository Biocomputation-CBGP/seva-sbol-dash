...
from utility.database.db_interface import DatabaseInterface
...
class AbstractSynBioHubInterface(DatabaseInterface):
    ...
    def get_names(self):
        sparql_query = self._query_builder.plasmid_names()
    ...
    def sequence(self, sequence):
        get_uri = f'{self.base}search/globalsequence={sequence}&'
        try:
        ...
        return {rdflib.URIRef(c["uri"]): c["percentMatch"] for c in contents}
...
class QueryBuilder:
    ...
    def plasmid_names(self):
        spql_string = f'{self.prefixes}\nSELECT DISTINCT ?subject\n'
        spql_string += '''  WHERE {?subject rdf:type sbol2:ComponentDefinition .
           <http://sevahub.es/public/Canonical/Canonical_collection/1> sbol2:member ?subject .
           FILTER NOT EXISTS  {?x sbol2:definition ?subject}}'''
        return spql_string
